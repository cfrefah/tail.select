...
;(function(window){
     ...
     */
    tailSelect.defaults = {
        width:              null,           // The used width, use 'auto' to calc the width from the source <select> element.
        height:             null,           // The used height
        classNames:         null,           // Additional classNames for the new select field, use "True" to copy the <select> class Names
        placeholder:        null,           // A Placeholder (or use placeholder="Your Placeholder" on the <select> element)
        deselect:           false,          // DeSelect on Single selection fields
        animate:            false,          // True to enable an animated open / close sequenze
        openAbove:          null,           // True to open the dropdown field always above the select field, false for the opposite and null for an auto.deterction.
        stayOpen:           false,          // True to keep the dropdown field on single-select fields open after selection
        startOpen:          false,          // True to open the dropdown field directly after the init.
        multiple:           false,          // Multiple selection field (or use multiple="multiple" on the <select> element)
        multiLimit:         -1,             // The Number of selectable items
        multiShowCount:     true,           // Show how many items has been selected
        multiContainer:     false,          // Move selected items to an container (use an selector or TRUE to use the selection field)
        descriptions:       false,          // True to enable option-descriptions, False to disable it
        items:              {},             // Add additional items as 'value' => 'name' or 'name' => {}
        sortItems:          false,          // Use "ASC", "DESC" or a callback function to sort the items object
        sortGroups:         false,          // Use "ASC", "DESC" or a callback function to sort the optgroups
        search:             false,          // True to enable the search field, False to disable it
        searchFocus:        true,           // Sets the Focus to the search field, when the dropdown opens
        searchMarked:       true,           // True to <mark> the search term within the option text.
        hideSelect:         true,           // True to hide the base <select> element
        hideSelected:       false,          // True to hide selected options within the dropdown (CSS-only!).
        hideDisabled:       false,          // True to hide disabled options within the dropdown (CSS-only!).
        bindSourceSelect:   false,          // Event-Binding to the Source <select> element.
    };
