/*
 *	TAIL.SELECT (DEVELOPMENT VERSION)
 *	A jquery extension to costumize and extend HTML Select fields.
 *	Written for the Dexs BD Framework and the Dexs PM System.
 *
 *	@author			SamBrishes, PYTES.NET
 *	@author_url		http://www.sambrishes.net, http://www.pytes.net
 *	@requires		jQuery
 *
 *	@copyright		Copyright © 2014 - pytesNET
 *	@license		MIT License
...
(function(){
	 ...
	 *	@since	0.2.0
	 *
	 *	@param	bool	True -> Mobile, False -> Supported.
	 *
	 *	@return	bool	True if a mobile (or supported) browser, False if not.
	 */
	tailSelect.prototype.browser = function(mobile){
		if(mobile === true){
			...
			/* OPERA 11 - 12 (uncomment this section to support Opera 11 and 12) */
			/*
	...
	/*
	 *	SYSTEM :: REGISTER A NEW ELEMENT
	 *	@since	0.1.0
	 *	@update	0.1.1	-	jQuery 1.4.0 Fix
	 *	@update	0.2.0
	 *
	 *	@param	object	The original select element
	 *	@param	object	The configuration object
	 *
	 *	@return	multi	The added element as object or false on failure.
	 */
	...
	/*
	 *	SYSTEM :: CREATE TAIL-SELECT FIELD
	 *	@since	0.2.0	-	Replaces the "render" function
	 *
	 *	@param	string	The tmsid of the element
	 */
	tailSelect.prototype.create = function(id){
		...
		// SELECT -> INPUT ELEMENT -> OPTIONS LIST
		...
		// SELECT -> INPUT ELEMENT -> INPUT PLACEHOLDER
		...
		// SELECT -> INPUT ELEMENT -> PULL ACTION
	...
	/*
	 *	SYSTEM :: (RE)LOAD TAIL-SELECT ITEM LIST
	 *	@since	0.1.0
	 *	@update	0.1.1	-	jQuery 1.4.0 Fix
	 *	@update	0.2.0
	 *
	 *	@param	string	The tmsid of the element
	 *
	 *	@return object	The option-items
	 */
	tailSelect.prototype.reload = function(id){
		...
		for(var i = 0; i < $(list).find("option").length; i++){
			...
			var single = false;			// To prevent multiple selections on single select fields
			items[i] = {
				"label":	"",
				"value":	"",
				"group":	false,
			...
			// IE.9 + jQuery 1.6.0 FIX 	=	$(data.selector).val() == $(item).attr("value")
	...
	/*
	 *	SYSTEM :: RENDER ITEM LIST
	 *	@since	0.2.0	-	Replaces a part of the "render" function
	 *
	 *	@param	string	The tmsid of the element
	 *	@param	object	A object list with all items
	 */
	...
	/*
	 *	SYSTEM :: BIND ACTIONS ON THE TAIL.SELECT FIELD
	 *	@since	0.2.0	-	Replaces the "bindActions" function
	 *
	 *	@param	string	The tmsid of the element
	 */
	 ...
	 *	@since	0.2.0	-	Replaces the "action_open", "action_click" and 
	 *						"action_closeSelect" functions
	 *
	 *	@param	string	The tmsid of the element
	 *	@param	string	New status
	 *	@param	string	The dependent list-item
	 */
	 ...
	 *	@since	0.1.0
	 *	@update	0.1.1	-	jQuery 1.4.0 Fix
	 *						Remove ' and " in the value to prevent errors.
	 *	@update	0.2.0
	 *
	 *	@param	string	The tmsid of the element
	 *	@param	object	The dependent list item
	 */
	tailSelect.prototype.action_select = function(id, item){
		...
		if(data.options.multiple === false){
			...
			if($(data.objects.list).find("li[data-selected='true']").length > 0){
				$.each($(data.objects.list).find("li[data-selected='true']"), function(index){
					_this.action_deselect(id, $(this).eq(index));	// If more options are selected for whatever reason D:
				});
			};
		};
		...
		if(typeof($.prop) !== "undefined"){
			$(option).prop("selected", true);	// jQuery 1.6 Fix
		};
	 ...
	 *	ACTION :: DESELECT
	 *	@since	0.1.0
	 *	@update	0.1.1	-	Remove ' and " in the value to prevent errors.
	 *	@update	0.2.0
	 *
	 *	@param	string	The tmsid of the element
	 *	@param	object	The dependent list item
	...
	tailSelect.prototype.action_deselect = function(id, item){
		...
		var option = $(data.selector).find("option[data-option='" + $(item).attr("data-number") + "']");
		if(typeof($.prop) !== "undefined"){
			$(option).prop("selected", false); //jQuery 1.6 Fix
		};
		...
		// SINGLE SELECT FIELD
		if(data.options.multiple === false){
			// DESELECT on single select fields
	...
	/*
	 *	ACTION :: PLACEHOLDER
	 *	@since	0.2.0	-	Replaces the "action_showCount" function and a
	 *						part of the "action_checkLimit" function
	 *
	 *	@param	string	The tmsid of the element
	 *	@param	object	The dependent list item
	 */
	...
	/*
	 *	CHECK :: IS LIMIT
	 *	@since	0.2.0	-	Replaces a part of the "action_checkLimit" function
	 *
	 *	@param	string	The tmsid of the element
	 *
	 *	@param	return	True if the limit is reached, False if not.
	 */
	...
	/*
	 *	FUNCTIONS
	 *	@since	0.2.0	-	Replaces "actions" functions
	 *
	 *	@param	object	The tail.Select element
	 *	@param	string	A valid action
	 *	@param	multi	string or array
	 *	@param	multi	string or array
	 */
	tailSelect.prototype.functions = function(selector, action, option, value){
		...
		// OPEN OR CLOSE tail.SELECT
		if(action === "open" || action === "close"){
			this.change_status(id, action, false);
			return true;
		};
		
		// DIS OR ENABLE OPTION(S)
